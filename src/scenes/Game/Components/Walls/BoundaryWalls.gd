class_name BoundaryWall extends Node2D
...
func getLevelWidth() -> float:
	return _right_wall.position.x - _left_wall.position.x

func getLeftWallPosition() -> Vector2:
	return _left_wall.position
