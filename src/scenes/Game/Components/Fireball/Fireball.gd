...
@export var strength_fall_off_power : int = 1
...
const STRENGTH_MULTIPLIER : float = 150
...
func _onCollision(_collided : Node) -> void:
	var pos : Vector2 = global_position - linearVelocity * 0.15
	...
	for body in bodies:
		if _getRayCast(body) == body:
			if body is Character: 
				var dir : Vector2 = (_ray_cast.get_collision_point() - pos).normalized()
				body._onFireball(dir * explosion_strength)
			elif body is RigidBody2D:
				var diff : Vector2 = _ray_cast.get_collision_point() - pos
				...
				if body is Platform:
					force *= 15
				body.apply_impulse(direction * force, _ray_cast.get_collision_point() - body.global_position)

	await get_tree().create_timer(0.4).timeout
	self.queue_free()

##Finds the first body on the path there
func _getRayCast(body : Node2D):
